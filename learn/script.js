...
//------------------------------------
//___Модули теория Структура папок__//
//используем модули для удобного переиспользования

//Корневой модуль
//Корневой модуль необходим для запуска приложения Angular.Этот модуль загружает все корневые компоненты и другие модули.Корневые модули альтернативно известны как AppModule и создаются в папке / src / app.

//Модуль Feature
//Модуль Feature, как следует из названия, реализует определенную функцию вашего приложения Angular
//изолировать логику по отдельным модулям например все что относиться user

//Общий модуль
//Далее по списку идут общие модули.Создание общих модулей в проекте Angular помогает организовать и упорядочить код.Вы можете собрать все часто используемые компоненты, трубы и директивы в одном модуле, а затем импортировать весь модуль в любую часть вашего приложения.
//shared -> кнопки / header ... переиспользуемые компоненты или декораторы / pipe

//Основной модуль
//И последний, но не по значению, модуль ядра.Основной модуль - это модуль, созданный для определения общих сервисов вашего приложения Angular.Сервисы, определенные в основном модуле, определяются как один экземпляр.Этот модуль импортируется из основного модуля, так как он содержит одноразовые сервисы, которые может использовать любой элемент вашего приложения Angular.Вы должны импортировать основной модуль только из корневого модуля.
//в core храним -> service / interceptor / model (интерфейсы / типизация)

//style gude / best practise - можно найти в real world example (проект на гите где используеться лучшие практики)

@NgModule({
  //регестрируем компоненты  и т.д.
  declarations: [App, AppComponent, ChildComponent, PageNotFoundComponent, UsersComponent, ProfileComponent],

  //импортировать наши модули
  imports: [
    BrowserModule,
    FormsModule,
    HttpClientModule,
  ...
  //с какой компоненты начать приложения
  bootstrap: [App],
})
export class AppModule { }
