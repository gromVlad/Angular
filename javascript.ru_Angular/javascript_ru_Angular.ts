...
//_Пример Host для понимание
//есть родительский компонент с дерективой и внутри дерективы провайдер а в дочерней дерективе мы инжектируем сервис из родительльской,если мы теперь украсим дочернею директиву с @Host то она работает также как и раньше , но если мы удалим провайдера из родительской дерективы то будет ошибка так как он ограничивает область разрешения сервиса в области просмотра не выходит за рамки хостинга, да же если выше и будет нужный провайдер.Тоесть родительская деректива становиться последним местам где эта зависемость могла быть предоставлена и разрешена.
...
//В этом примере компонент ParentComponent предоставляет зависимость FlowerService с помощью providers. Родительская директива ParentDirective использует конструктор для внедрения зависимости FlowerService. Дочерняя директива ChildDirective также использует конструктор для внедрения зависимости FlowerService, но с аннотацией @Host().

//Когда дочерняя директива создается, она вызывает конструктор и получает экземпляр FlowerService из родительской директивы. Это работает, потому что родительская директива находится в области видимости дочерней директивы.

//Теперь предположим, что мы удалим провайдер FlowerService из родительской директивы - Error: No provider for FlowerService!
//Это происходит потому, что аннотация @Host() ограничивает область разрешения зависимости областью видимости компонента-хоста. В данном случае компонент-хост — это родительская директива
